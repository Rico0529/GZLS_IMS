<template>
	<div class="main-content" :style='{"padding":"0","backgroundColor":"#F0F0F0","fontSize":"15px","overflow":"hidden"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">招聘岗位</label>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('zhaopinxinxi','投递简历')" @click="toudijianliCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								投递简历
							</el-button>
		...
		<toudijianli-cross-add-or-update v-if="toudijianliCrossAddOrUpdateFlag" :parent="this" ref="toudijianliCrossaddOrUpdate"></toudijianli-cross-add-or-update>
...
<script>
	...
	import toudijianliCrossAddOrUpdate from "../toudijianli/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				toudijianliCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			toudijianliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.toudijianliCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.toudijianliCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.toudijianliCrossaddOrUpdate.init(row.id,type);
				});
			},
